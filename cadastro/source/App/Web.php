...
class Web extends Controller
{
        ...
        if (!empty($data['csrf'])) {
            ...
            if (request_limit("weblogin", 3, 60 * 5)) {
                $json['message'] = $this->message->error(
                    "Você já efetuou 3 tentativas, esse é o limite. Por favor, aguarde 5 minutos para tentar novamente!"
                )->render();
                echo json_encode($json);
                return;
            }
    ...
    public function register(?array $data): void
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if (!is_cpf($data["cpf"])) {
                $json['message'] = $this->message->warning("CPF informado não é valido!")->render();
                echo json_encode($json);
                return;
            }
            ...
            if (!empty($data["cpf"])) {
                if (!csrf_verify($data)) {
                    $json['message'] = $this->message->warning("CPF já está em uso, ou cadastrado.")->render();
                    echo json_encode($json);
                    return;
                }
            }
    ...
    public function success(array $data): void
    {
        ...
        echo $this->view->render("optin", [
            "head" => $head,
            "data" => (object)[
                "title" => "Tudo pronto. Você já pode Acessar :)",
                "desc" => "Bem-vindo(a) ao MegaZorde!",
